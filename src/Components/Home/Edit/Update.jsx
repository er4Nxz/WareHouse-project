...
import Items from "./ItemsPlace/Items";
import axios from "axios";
import Swal from "sweetalert2";

const Update = ({ state }) => {
  const [place] = useFetch("http://localhost:3000/Place");
  ...
  useEffect(() => {
    ...
    setDescrption(state.descrption);
  ...
  const handleSubmit = (e) => {
    ...
    const updateData = async () => {
      try {
        await axios.put(`http://localhost:3000/Components/${state.id}`, {
          title,
          descrption,
          count,
          image,
        ...
        <div className="max-w-lg mx-auto">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 shadow-lg">
            ...
            >
              ...
              >
                <FaSave className="inline ml-1" /> بروز رسانی محصول
              </button>
            </form>
          </div>
        </div>
